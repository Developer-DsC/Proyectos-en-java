...
public class ArbolAVL<T extends Comparable<T>> extends ArbolBB<T> {
	...
	/**
	 * Constructor de un árbol AVL de nombre <em>nombre</em> y con un nodo raíz con
	 * el <em>dato</em>
	 * 
	...
	/**
	 * Constructor de un árbol AVL sin nombre y con un nodo raíz con el
	 * <em>dato</em>
	 * 
	 ...
	 * @return Nodo que contiene el objeto si lo encuentra, y <em>null</em> si no lo
	 *         encuentra
	 */
	...
	private NodoArbol<T> buscarNodo(NodoArbol<T> arbol, T dato) {
		if (arbol != null) {
			if (arbol.getClave().compareTo(dato) == 0)
				return arbol;
			else if (arbol.getClave().compareTo(dato) > 0) {
	...
	/**
	 * Inserta un nuevo nodo en el árbol AVL a partir de un subárbol. Si el objeto
	 * del nuevo nodo ya existe en el árbol, este no se inserta. Si el nuevo nodo
	 * produce un desbalanceo, se producen las rotaciones necesarias para balancear
	 * el árbol. Esto generará una variación en el subárbol que se rota y por tanto
	 * el método entrega una nueva raíz de dicho subárbol.
	 * 
