...
public class AlgoritmosGrafos {
	...
	/**
	 * Muestra información importante del grafo dado:
	 * Nombre
	 * Si es dirigido
	 * Grados de entrada y salida de los vértices
	 * Orden
	 * Tamaño
	...
	public static void estadoGrafo(Grafo grafo) {
		...
		for(int i=0; i<grafo.obtenerNumVertices(); ++i)
			System.out.println("Vértice " + i + ": GIn= " + grafo.gradoIn(i) + 
	 ...
	 * @param ma Tipo de implementación deseada: true genera un grafo con Matriz de Adyacencias, con false genera un grafo con Listas de Adyacencias
	 * @return El grafo generado a partir de la matriz de adyacencias
	 */
	public static Grafo getGrafo(int[][] mAdy, boolean ma){
		Grafo grafo;
		int n = mAdy.length;
		if(ma) //crea un grafo según el tipo, se indica que es dirigido, pero es la disposición de los arcos es lo que lo define.
	...
	/**
	 * Método que retorna la multiplicación de dos matrices de enteros C=AB
	...
	/**
	 * Método para determinar el número de caminos de una longitud dada entre dos vértices
	 * @param g Grafo
	 * @param m Longitud de los caminos
	 * @param i Vértice origen
	 * @param j Vértice destino
	 * @return Número de caminos de longitud m desde i y hasta j, o -1 si el grafo no está implementado
	 */
	...
	/**
	 * Método recursivo para determinar el número de caminos de una longitud dada entre dos vértices
	 * @param g Grafo
	 * @param m Longitud de los caminos
	 * @param i Vértice origen
	 * @param j Vértice destino
	...
	/**
	 * Método que retorna la matriz de caminos de longitud m de un Grafo dado
	 * @param g Grafo
	 * @param m Longitud de los caminos
	 * @return Matriz con el número de caminos de longitud m para cada par de vértices
	 */
	...
	/**
	 * Método que retorna la matriz de caminos de longitud m 
	...
	/**
	 * Método que determina si el Grafo grafo dirigido tiene ciclos
	...
	public static boolean tieneCiclos(Grafo grafo) {
		...
		if(!grafo.esNoDirigido()) { //solo entra si el grafo es dirigido
			ady = matrizAdy(grafo);
			mc = matrizCaminos(grafo);
			for(int i=0; i<mc.length; ++i)
				if(ady[i][i]==0 && mc[i][i]!=0) { //ciclos que no sean bucles
					resul = true;
					i = mc.length;
				}
		}
	...
	/**
	 * Determina el número de bucles que tiene el Grafo grafo
	 * @param grafo Grafo cualquiera implementado por Listas o Matriz de adyacencias
	 * @return El número de bucles del grafo
	 */
	...
	public static int[][] matrizT(int[][] matriz) {
		int numFilas = matriz.length; //número de filas de la matriz
		int numCols;
		int[][] mT = new int[numFilas][];

		for (int fila = 0; fila < numFilas; ++fila) {
			numCols = matriz[fila].length; //número de columnas por fila, no necesariamente es regular
	 ...
	 * @return El grafo inverso como Grafo o null si no es dirigido. Quien use el método deberá hacer una casting hacia GrafoMA o GrafoLA
	 */
	...
	/**
	 * Obtiene el tipo de implementación del grafo dado
